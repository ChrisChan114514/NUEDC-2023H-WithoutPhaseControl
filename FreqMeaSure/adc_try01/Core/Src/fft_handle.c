...
#define SIN_JUDGE  210
#define FOCUS_JUDGE_3 40                                    //40个索引内（40*f0频率内），能量直接集中到三倍基波分量
#define TYPE_JUDGE_NUM 2                                 //波形类型的削平参数
...
char pData_C[100]="@b051021d4#";     //UART-TX数据
FRE FREa,FREb;

// 比较两个字符串在指定长度内是否完全相同
int compare_substrings(char *str1,  char *str2, int index1, int index2) {
    // 检查输入参数的有效性
    if (str1 == NULL || str2 == NULL || index1 < 0 || index2 < index1) {
        return 0;
    }
...
// 将一个字符串在指定长度内的内容复制到另一个字符串
void copy_substring(char *dest, char *src, int index1, int index2) {
    // 检查输入参数的有效性
    if (dest == NULL || src == NULL || index1 < 0 || index2 < index1) {
        return;
    }
...
void fft_waveform_check_H(void)	//为信号分离H题服务的波形判断及频率测量函数
{
	
	static int UART_NUM=3; //UART传输次数 改变一次测量值应该发送3次（保险起见）
	static char PRE_pData[100]="@a00a00#";
	...
	//判定波形
	if (fabs(sortedfft_v[0]-sortedfft_v[1])>30) //两种波
	{
...
//	printf("A2:%f,F2:%f\n",fft_outputbuf[sortedfft_i[1]] ,F2);      //最重要的数据，其他prinf可以移去
	if (F1<F2)
	{
		FREa.FREC=F1;
		FREb.FREC=F2;
...
//	printf("bF:%fHZ\n",FREb.FREC);
	...
	if (CLKsum==TYPE_JUDGE_NUM)
	{
		...
		TYPEsumB=0;
		sprintf(SPR_STR,"A_TYPE:%1d",FREa.TYPEpro);
		lcd_show_string(0,10,200,16,32,SPR_STR,WHITE);
		sprintf(SPR_STR,"B_TYPE:%1d",FREb.TYPEpro);
        lcd_show_string(0,50,250,16,32,SPR_STR,WHITE);
		pData_C[0]='@';
		//增加波形选择逻辑 
		if (OUT_mode==1) //输出A波
		{
			pData_C[1]=(FREa.TYPEpro==0)?'a':'b';
			pData_C[4]=(FREb.TYPEpro==0)?'a':'b';
			pData_C[2]=(FREa.FRECint/5000-4)/10+48;
			pData_C[3]=(FREa.FRECint/5000-4)%10+48;
			pData_C[5]=(FREb.FRECint/5000-4)/10+48;
			pData_C[6]=(FREb.FRECint/5000-4)%10+48;
			lcd_show_string(250,300,250,16,32,"CurrentlyOUT:A",WHITE);
		}
		else //输出B波
		{
			pData_C[1]=(FREb.TYPEpro==0)?'a':'b';
			pData_C[4]=(FREa.TYPEpro==0)?'a':'b';
			pData_C[2]=(FREb.FRECint/5000-4)/10+48;
			pData_C[3]=(FREb.FRECint/5000-4)%10+48;
			pData_C[5]=(FREa.FRECint/5000-4)/10+48;
			pData_C[6]=(FREa.FRECint/5000-4)%10+48;
			lcd_show_string(250,300,250,16,32,"CurrentlyOUT:B",WHITE);
		}
		...
		//优化printf,有变化了才进行通信
		if (compare_substrings(pData_C,PRE_pData,0,7)==0)
		{
			//printf("%c%c%c%c%c%c%c%c\n",pData_C[0],pData_C[1],pData_C[2],pData_C[3],pData_C[4],pData_C[5],pData_C[6],pData_C[7]);
			UART_NUM=3;
		}
		copy_substring(PRE_pData,pData_C,0,7);
...
//	printf("bT:%d,bF:%fHZ\n",FREb.TYPEpro,FREb.FREC);
	...
	sprintf(SPR_STR,"A_FREQ:%3dkHZ",FREa.FRECint/1000);
	lcd_show_string(300,10,200,16,32,SPR_STR,YELLOW);
	

	sprintf(SPR_STR,"B_FREQ:%3dkHZ",FREb.FRECint/1000);
	lcd_show_string(300,50,200,16,32,SPR_STR,YELLOW);
	...
	//HAL_UART_Transmit(&huart3, (uint8_t *)pData_C, 30, 0xFFFF);
	//printf("%s\n",pData_C);
